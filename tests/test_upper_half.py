...
class TestUpperHalfManifold(sympa.tests.TestCase):
    ...
    def test_distance_is_symmetric_real_pos_imag_pos(self):
        x = self.manifold.projx(get_random_symmetric_matrices(10, 3) * 10)
        y = self.manifold.projx(get_random_symmetric_matrices(10, 3) * 10)
    ...
    def test_distance_is_symmetric_real_pos_imag_neg(self):
        x = get_random_symmetric_matrices(10, 3) * 10
        x = self.manifold.projx(sm.stick(sm.real(x), sm.imag(x) * -1))
        y = self.manifold.projx(get_random_symmetric_matrices(10, 3) * 10)
    ...
    def test_distance_is_symmetric_real_neg_imag_neg(self):
        x = get_random_symmetric_matrices(10, 3) * 10
        x = self.manifold.projx(sm.stick(sm.real(x) * -1, sm.imag(x) * -1))
        y = self.manifold.projx(get_random_symmetric_matrices(10, 3) * 10)
    ...
    def test_distance_is_symmetric_real_neg_imag_pos(self):
        x = get_random_symmetric_matrices(10, 3) * 10
        x = self.manifold.projx(sm.stick(sm.real(x) * -1, sm.imag(x)))
        y = self.manifold.projx(get_random_symmetric_matrices(10, 3) * 10)
